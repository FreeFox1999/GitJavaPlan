...
REM ---- Show env after clearing (requested) ----
...
echo MAVEN_OPTS=%MAVEN_OPTS% >> "%LOG%"
echo MAVEN_ARGS=%MAVEN_ARGS% >> "%LOG%"
REM [FIX 1] Was "JAVA TOOL OPTIONS" (missing underscores) -> JAVA_TOOL_OPTIONS
echo JAVA_TOOL_OPTIONS=%JAVA_TOOL_OPTIONS% >> "%LOG%"
REM [FIX 2] Was missing >> "%LOG%" redirect on _JAVA_OPTIONS line
echo _JAVA_OPTIONS=%_JAVA_OPTIONS% >> "%LOG%"
echo JDK_JAVA_OPTIONS=%JDK_JAVA_OPTIONS% >> "%LOG%"

REM ================================================================
REM 0c) Set local envs for THIS SESSION (requested)
REM ================================================================
REM Prefer portable tools on PATH
set "PATH=%JAVA_HOME%\bin;%MAVEN_HOME%\bin;%PATH%"

REM Set MAVEN_OPTS trustStore to the dynamic absolute cert path.
REM IMPORTANT: Do NOT use intermediate variable inside parentheses without delayed expansion.
REM We set MAVEN_OPTS directly from the FOR variable to avoid expansion timing bugs.
set "MAVEN_OPTS="
if exist "%CERT_FILE%" (
REM [FIX 3] Was: set "MAVEN_OPTS=-Djavax.net.ssl.trustStore=\"%%~fI\""
REM Escaped quotes \" inside set "..." break CMD parsing for paths with spaces.
REM Correct: %%~fI resolves the full absolute path without extra quoting needed in set.
  for %%I in ("%CERT_FILE%") do set "MAVEN_OPTS=-Djavax.net.ssl.trustStore=%%~fI"
) else (
  echo [WARN] Certificate file not found: "%CERT_FILE%" >> "%LOG%"
)

REM ---- Show local envs after setting (requested) ----
echo.
...
echo.
echo --- Maven Version ---
REM [FIX 4] Was "echo call ..." which only PRINTED the command, never executed it.
REM Removed the spurious "echo" keyword so Maven actually runs.
call "%MVN_CMD%" -v
...
REM NOTE: MAVEN_OPTS does NOT affect java -jar.
REM ================================================================
echo ===== Running jar =====
echo "%JAVA_EXE%" -jar "%JAR%" --association "%ASSOC%" --p1 %P1% --p2 %P2% --p3 %P3% --p4 %P4% --p5 %P5% --out "%OUT%"
echo.

powershell -NoProfile -Command "& { & '%JAVA_EXE%' -jar '%JAR%' --association '%ASSOC%' --p1 %P1% --p2 %P2% --p3 %P3% --p4 %P4% --p5 %P5% --out '%OUT%' 2>&1 | Tee-Object -FilePath '%LOG%' -Append; exit $LASTEXITCODE }"
if errorlevel 1 (
